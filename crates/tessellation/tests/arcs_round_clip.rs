...
fn options(join: LineJoin) -> StrokeOptions {
...
fn round_clip_preserves_unclipped_arcs_and_round_fallbacks_exactly() {
    for curvature in [-0.4, -0.6, 0.1] {
        ...
        let standard = mesh(&path, &options(LineJoin::Arcs));
        let rounded = mesh(&path, &options(LineJoin::ArcsRound));
...
fn rounded_cut_adds_geometry_for_both_turn_directions_and_keeps_original_vertices() {
    for mirror in [-1.0, 1.0] {
        ...
        let options = options(LineJoin::Arcs).with_miter_limit(1.6);
        let standard = mesh(&path, &options);
        let rounded = mesh(&path, &options.with_line_join(LineJoin::ArcsRound));
...
fn round_cut_vertices_keep_join_metadata_instead_of_cap_center() {
    ...
    let options = options(LineJoin::ArcsRound).with_miter_limit(1.6);
...
fn mesh_positions_for_standard(
    ...
    mesh(path, &options.with_line_join(LineJoin::Arcs))
}
...
fn straight_segment_miter_cuts_are_rounded_too() {
    ...
    let options = options(LineJoin::Arcs).with_miter_limit(2.0);
    let standard = mesh(&path, &options);
    let rounded = mesh(&path, &options.with_line_join(LineJoin::ArcsRound));
    assert!(rounded.vertices.len() > standard.vertices.len());
}
...
fn zero_and_subunit_limits_produce_finite_rounded_geometry() {
    ...
    for limit in [0.0, 0.1, 0.5, 1.0, 1.6] {
        let style = options(LineJoin::Arcs).with_miter_limit(limit);
        let standard = mesh(&path, &style);
        let rounded = mesh(&path, &style.with_line_join(LineJoin::ArcsRound));
        assert!(rounded
            .vertices
            .iter()
            .all(|p| p.x.is_finite() && p.y.is_finite()));
...
fn asymmetric_curved_miter_cut_gets_a_round_tip() {
    ...
    builder.end(false);
    let path = builder.build();
    let style = options(LineJoin::Arcs)
...
fn opposite_parallel_rectangle_gets_a_round_far_edge() {
    ...
    builder.end(false);
    let path = builder.build();
    let style = options(LineJoin::Arcs).with_miter_limit(3.0);
    let standard = mesh(&path, &style);
    let rounded = mesh(&path, &style.with_line_join(LineJoin::ArcsRound));
    let old_x = standard
        .vertices
...
#[test]
fn default_join_remains_miter() {
    assert_eq!(StrokeOptions::default().line_join, LineJoin::Miter);
}

#[cfg(feature = "serialization")]
#[test]
fn existing_options_keep_their_serialized_shape() {
    ...
    assert_eq!(old.line_join, LineJoin::Miter);
    assert_eq!(serde_json::to_string(&old).unwrap(), json);
}

#[cfg(feature = "serialization")]
#[test]
fn arcs_variants_round_trip_without_a_separate_clip_field() {
    for (join, name) in [(LineJoin::Arcs, "Arcs"), (LineJoin::ArcsRound, "ArcsRound")] {
        let options = options(join);
        let encoded = serde_json::to_value(options).unwrap();
        assert_eq!(encoded["line_join"], name);
        assert!(encoded.get("arcs_clip").is_none());
        assert_eq!(
            serde_json::from_value::<StrokeOptions>(encoded).unwrap(),
            options
        );
    }
}
...
#[test]
fn builder_can_enable_both_arcs_variants_for_open_and_closed_curves() {
    use lyon_path::builder::{Build, PathBuilder};

    for join in [LineJoin::Arcs, LineJoin::ArcsRound] {
        for closed in [false, true] {
            let curves = segments(-0.4);
            let mut path = Path::builder();
            path.begin(curves[0].from);
            for curve in curves {
                path.cubic_bezier_to(curve.ctrl1, curve.ctrl2, curve.to);
            }
            path.end(closed);
            let style = options(join).with_miter_limit(1.6);
            let expected = mesh(&path.build(), &style);
            let initial = style.with_line_join(LineJoin::Miter);
            let mut actual: VertexBuffers<_, u32> = VertexBuffers::new();
            let mut output = BuffersBuilder::new(&mut actual, |v: StrokeVertex| v.position());
            let mut tessellator = StrokeTessellator::new();
            let mut builder = tessellator.builder_with_attributes(0, &initial, &mut output);
            builder.set_line_join(join);
            builder.begin(curves[0].from, &[]);
            for curve in curves {
...
#[test]
fn buffered_join_keeps_its_rounding_when_builder_changes_style() {
    use lyon_path::builder::{Build, PathBuilder};

    let mut path = Path::builder_with_attributes(1);
    path.begin(point(-40.0, 0.0), &[1.0]);
    path.line_to(point(0.0, 0.0), &[1.25]);
    path.line_to(point(-30.0, 10.0), &[0.75]);
    path.end(false);
    let path = path.build();

    for (join, next_join) in [
        (LineJoin::Arcs, LineJoin::ArcsRound),
        (LineJoin::ArcsRound, LineJoin::Arcs),
    ] {
        for variable_width in [false, true] {
            let mut style = options(join).with_miter_limit(2.0);
            if variable_width {
                style = style.with_variable_line_width(0);
            }
            let expected = mesh(&path, &style);
            let mut actual: VertexBuffers<_, u32> = VertexBuffers::new();
            let mut output = BuffersBuilder::new(&mut actual, |v: StrokeVertex| v.position());
            let mut tessellator = StrokeTessellator::new();
            let mut builder = tessellator.builder_with_attributes(1, &style, &mut output);
            builder.begin(point(-40.0, 0.0), &[1.0]);
            builder.line_to(point(0.0, 0.0), &[1.25]);
            builder.set_line_join(next_join);
            builder.line_to(point(-30.0, 10.0), &[0.75]);
            builder.end(false);
            builder.build().unwrap();
            assert_eq!(
                actual.vertices, expected.vertices,
                "{join:?}, variable_width={variable_width}"
            );
            assert_eq!(actual.indices, expected.indices);
        }
    }
